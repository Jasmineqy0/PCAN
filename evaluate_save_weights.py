...
weights_path = 'visualization/'+dataset + '_weights'
if not os.path.exists(weights_path):
    os.mkdir(weights_path)

LOG_DIR = 'models'
output_file= RESULTS_FOLDER +'refine_results_'+dataset+'.txt'
model_file= "base.ckpt"
...
def get_latent_vectors(sess, ops, dict_to_process):
    ...
    for q_index in range(len(train_file_idxs)//batch_num):
        ...
        o1, o2, o3, weights=sess.run([ops['q_vec'], ops['pos_vecs'], ops['neg_vecs'], ops['weights']], feed_dict=feed_dict)

        weights = np.reshape(weights,[-1,4096,1])
        weights = np.array(weights)

        for i in range(11):
            pc_name = file_names[i].replace('/', '_')
            pc_name = pc_name.replace('.', '_')
            pc_sample_name = pc_name.split('_')[-2]
            weights[i].tofile(weights_path + '/' + pc_sample_name + '.bin')
